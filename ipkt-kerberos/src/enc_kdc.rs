...
use crate::aes_cts::{decrypt_aes256, string2key_aes256, KEY_USAGE_AS_REP_ENC_PART, KEY_USAGE_TGS_REP_ENC_PART};
use crate::crypto::ETYPE_AES256_CTS_HMAC_SHA1_96;
use crate::des_crypto::{
    decrypt_des3_cbc_sha1, decrypt_des_cbc_crc, decrypt_des_cbc_md5, string2key_des, string2key_des3,
    ETYPE_DES3_CBC_SHA1, ETYPE_DES_CBC_CRC, ETYPE_DES_CBC_MD5,
};
...
pub fn session_key_from_as_rep(
    ...
    match enc.etype {
        ...
        et => Err(crate::Error::Crypto(format!("unsupported AS-REP etype {et}"))),
    }
}
...
fn parse_session_key_from_enc_kdc_rep(plain: &[u8], etype: i32) -> Result<KerberosSessionKey> {
    ...
    if key.is_empty() {
        return Err(crate::Error::InvalidMessage("missing key in EncKDCRepPart".into()));
    }
